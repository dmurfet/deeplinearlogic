"""
Implements functions for reading and writing from and to the memory bank via weight vectors as in section 3.
"""
import autograd.numpy as np

#This function returns r_t equation (2)
def read(memoryObject, weight_vector):
    """
    The reading procedure as described in 3.1.
    weight_vector (w_t below) is a weighting over the rows such that:
        1. \sum_i w_t(i) = 1
        2. 0 \leq w_t(i) \leq 1, \for all i
    We return r_t = \sum_i w_t(i) M_t(i); M_t(i) is the i-th row of memory.
    """
    r_t = np.dot(weight_vector.T, memoryObject)

    return r_t

#This function returns equation (4), THINK about it when implementing the LSTM
# According to the paper, the erase/add decomposition was inspired by the forget/input gates in LSTM.
def write(memoryObject, w_t, e_t, a_t):
    """
    ...
    # will keep M_t[i,j].
    KEEP = np.ones(memoryObject.shape) - WTE

    # To complete erasure, multiply memory pointwise by KEEP
    newmemory = np.multiply(memoryObject, KEEP)
