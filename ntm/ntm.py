...
##################
# Multiple Pattern NTM Controller
#
# The multiple pattern NTM has four memory rings
#
#  - ring 1, as in the NTM
#  - ring 2, for patterns, like ring 2 of the Pattern NTM
#  - ring 3, for patterns, like ring 2 of the Pattern NTM
#  - ring 4, for switching between patterns
#
# Ring 4 has content size 2 and we use it as a logits to generate
# a distribution over whether to use the operator generated by ring 2
# or ring 3. This resulting linear combination of the two operators is
# then further interpolated as in the usual Pattern NTM, with the direct
# manipulation of the first ring by the controller.
    
class MultPatternNTM(RNNCell):
    """
    The main Multiple Pattern NTM code.
    """
    def __init__(self, num_units, input_size, controller_state_size,
                memory_address_sizes,memory_content_sizes, 
                powers, powers21, direct_bias):
    ...
    def __call__(self, input, state, scope, reuse=True):
        ...
        with tf.variable_scope(scope,reuse=reuse): 
            ...
            h0, r1, w1, r2, w2, r3, w3, r4, w4, M1, M2, M3, M4 = tf.split(state, [css, mas[0], mas[0], 
                                                        mas[1], mas[1],
                                                        mas[2], mas[2],
                                                        mas[3], mas[3],
                                                        mas[0] * mcs[0], 
            ...
            use_logits = [False, True, True, True]
            ...
            # ASSUME mcs[1] = len(powers21)
            Mr2_rot = tf.tensordot( Mr2, rot, [[1], [0]] ) # shape [batch_size, mas[0], mas[0]]
            r0_prime2 = tf.matmul( tf.reshape(r[0], [-1,1,mas[0]]), Mr2_rot )
            r0_prime2 = tf.reshape( r0_prime2, [-1,mas[0]] )

            Mr3_rot = tf.tensordot( Mr3, rot, [[1], [0]] ) # shape [batch_size, mas[0], mas[0]]
            r0_prime3 = tf.matmul( tf.reshape(r[0], [-1,1,mas[0]]), Mr3_rot )
            r0_prime3 = tf.reshape( r0_prime3, [-1,mas[0]] )
            
            # We use the fourth memory ring to interpolate between r0_prime2 and r0_prime3
            r0_alt = tf.matmul( tf.stack([r0_prime2, r0_prime3], axis=2), Mr4 )
            r0_alt = tf.reshape( r0_alt, [-1, mas[0]] )
            
            # Now interpolate between r0_alt and r_news[0] using interp
            interp_matrix = tf.stack([interp,
                                    tf.ones_like(interp,dtype=tf.float32) - interp],
                                    axis=1) # shape [-1,2,1]
            
            r0_new = tf.matmul( tf.stack([r_news[0], r0_alt], axis=2), interp_matrix )
            #                   [---- this is shape [-1,mas[0],2] ----] 
            ...
            state_new = tf.concat([h0_new, r_news[0], w_news[0], r_news[1], w_news[1],
                                        r_news[2], w_news[2], r_news[3], w_news[3],
                                        M_news[0], M_news[1], M_news[2], M_news[3]], 1)   
        return h0_new, state_new
        # the return is output, state