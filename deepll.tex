...
\]
The value of the RNN on $\bold{x}$ is computed by applying \eqref{eq:update_eqn} for $0 \le t \le T - 1$ to compute the vector $h^{(T)}$, and then feeding this into a second decoder neural network to output the final value \cite{sutskever2}. For the moment we do not specify the decoder. The point is that the evolution equation implicitly determines a piecewise-linear function
\begin{gather*}
\mathscr{I} \lto \operatorname{Func}(\mathscr{H}, \mathscr{H} )\,,\\
x \mapsto \sigma\big( H (-) + U x + B\big)\,,
\end{gather*}
which specifies how we may view inputs as transforming the internal state of the RNN. Modifications of the RNN architecture can be viewed as allowing more complicated (for example, piecewise-polynomial of higher degree) transformations of the internal state.

For example, compositional matrix-space models in NLP \cite{irsoy} add a new term built from an auxiliary learned linear mapping of the input space $\mathscr{I}$ into the space of linear operators on the state space $\End_{\mathbb{R}}(\mathscr{H})$. Our approach is similar, but we allow a particular class of non-linear maps $\mathscr{I} \lto \End_{\mathbb{R}}(\mathscr{H})$ selected by linear logic. The class of maps we consider are the denotations of proofs $\pi$ in intuitionistic linear logic of type
\be
\textbf{bint}_\alpha = {!}(\alpha \multimap \alpha) \multimap \big({!}(\alpha \multimap \alpha) \multimap (\alpha \multimap \alpha)\big)\,.
\ee
A full explanation of linear logic is not necessary. We mention only that under the Curry-Howard correspondence a proof $\pi$ of the above type can be thought of as a functional program taking two inputs (of type $\alpha \multimap \alpha$) and giving an output of the same type.

The \emph{denotation} of such a proof is a polynomial function
\[
\den{\pi}: \End_{\mathbb{R}}(\mathscr{H}) \times \End_{\mathbb{R}}(\mathscr{H}) \lto \End_{\mathbb{R}}(\mathscr{H})\,.
\]
where $\End_{\mathbb{R}}(V)$ denotes the space of linear operators on $V$.\footnote{To explain what we mean by a ``polynomial function'', suppose $\mathscr{H} = \mathbb{R}^r$ so that $\End_{\mathbb{R}}(\mathscr{H})$ is the space of matrices $M_r(\mathbb{R})$. Then for all $1 \le i,j \le r$, the denotation $\den{\pi}_{i,j}$ is a function of pairs of matrices $X,Y \in M_r(\mathbb{R})$ which is a polynomial in the entries $X_{ab}, Y_{cd}$.} For every binary sequence $S \in \{0,1\}^*$ there is a corresponding proof $t_S$ of type $\textbf{bint}_\alpha$, and for example the denotation of $t_{01101}$ is the polynomial function
\be
\den{t_{01101}}(X,Y) = X Y Y X Y\,.
\ee
It is these polynomial functions that we will couple into the RNN. 

Let $\Pi$ be a finite sequence of proofs $\Pi = (\pi_1,\ldots,\pi_k)$ of $\textbf{bint}_\alpha$. When we say that we allow the RNN controller to call this library of functional programs, what we actually mean is that it can predict a linear combination $p^{(t)}$ of the polynomial programs $\den{\pi_i}$ to be applied to its internal state, as follows:

\begin{definition} The \emph{linear logic} RNN associated to $\Pi$ has weight matrices $H,U,B$ as in the standard RNN, together with new weight matrices $V, Q, C$, and an evolution equation defined in two parts:
\begin{align}
p^{(t+1)} &= \sigma\big( Q h^{(t)} + C \big)\,,\\
h^{(t+1)} &= \sigma\Big( \sum_i p^{(t+1)}_i \den{\pi_i}( Vx^{(t)}, Vx^{(t+1)})\big( h^{(t)} \big) + H h^{(t)} + U x^{(t+1)} + B\Big)\,.
\end{align}
Here $V$ is a linear map $\mathscr{I} \lto \End_{\mathbb{R}}(\mathscr{H})$ and $p^{(t)} \in \mathbb{R}^k$ for all $t$.
\end{definition}
...
\begin{remark} To make it explicit, suppose $\mathscr{I} = \mathbb{R}^{n_I}$ and $\mathscr{H} = \mathbb{R}^{n_H}$, then $H \in M_{n_H \times n_H}(\mathbb{R})$, $U \in M_{n_H \times n_I}(\mathbb{R})$, $B \in M_{n_H \times 1}(\mathbb{R})$, $V \in M_{n_H^2 \times n_I}(\mathbb{R})$, $Q \in M_{k \times n_H}(\mathbb{R})$, $C \in \mathbb{R}^k$. Since $n_H^2$ may be large, we can act on a subspace by choosing a decomposition $\mathscr{H} = \mathscr{H}_0 \oplus \mathscr{H}_1$ and letting $\rho: \mathscr{H} \lto \mathscr{H}_1, \iota: \mathscr{H}_1 \lto \mathscr{H}$ be respectively be the projection to and inclusion of the subspace $\mathscr{H}_1$. Then we replace the evolution equation by
\be
h^{(t+1)} = \sigma\Big( \sum_i p^{(t+1)}_i \iota \den{\pi_i}( Vx^{(t)}, Vx^{(t+1)})\big( \rho h^{(t)} \big) + H h^{(t)} + U x^{(t+1)} + B\Big)\,.
\ee
with $\den{\pi}$ now a function $\End_{\mathbb{R}}(\mathscr{H}_1) \times \End_{\mathbb{R}}(\mathscr{H}_1) \lto \End_{\mathbb{R}}(\mathscr{H}_1)$.

If we write $n_H' = \dim(\mathscr{H}_1)$ then the number of weights in the linear logic RNN is
\be
n_H^2 + n_H n_I + n_H + (n_H')^2 n_I + kn_H + k\,,
\ee
where $k$ is the number of proofs we are indexing over.
\end{remark}
