...
\item types $B_1,\ldots,B_s$, called the \emph{input types};
\item a type $A$ with $\den{A} = \mathscr{H}$, the hidden state space;
\item a proof $\underline{\mathrm{master}}$ of the sequent
\be
{!} P_1, \ldots, \, {!}P_r,\, {!}(B_1 \multimap B_1), \ldots,\, {!}(B_s \multimap B_s) \vdash A \multimap A\,.
\ee
\item Finite-dimensional subspaces for $1 \le i \le r$ and $1 \le j \le s$
\be
\mathscr{P}_i \subseteq \den{P_i}\,, \quad \mathscr{B}_j \subseteq \den{B_j \multimap B_j} = \End_{\mathbb{R}}(\den{B_j})
\ee
which are spanned by the denotations of linear logic proofs. We refer to the elements of $\mathscr{P}_i$ as \emph{command vectors} and elements of $\mathscr{B_j}$ as \emph{input operators}.
\end{itemize}
\end{definition}
From this data we obtain the function
\be
\den{\underline{\mathrm{master}}}_{nl}: \mathscr{P}_1 \times \cdots \times \mathscr{P}_r \times \mathscr{B}_1 \times \cdots \times \mathscr{B}_s \lto \End_{\mathbb{R}}(\mathscr{H})
\ee
by restriction. At each time step the RNN will generate from the hidden state a sequence of command vectors, and from the current and previous inputs a sequence of input operators. This is done via weight matrices $\{W_i\}_{1 \le i \le r}$ and $\{V_j\}_{1 \le j \le s}$ which determine linear maps
\begin{align*}
...
\begin{example}[(Stack-augmented RNN)] We can emulate the stack-augmented RNN \cite{joulin} with appropriate choices of the defining data of the LLRNN. We begin with a decomposition of the hidden state space as $\mathscr{H} = \mathscr{H}_0 \oplus \mathscr{S}$ where the ``memory stack''
\be
