...
- **Version 5** (*current version*). The interpolation between direct access and indirect access in the Pattern NTM has been switched to hard sigmoid, with a bias towards direct access. We are now using hard sigmoid everywhere there was a sigmoid in `v4`.
