...
- **Version 5** (*current version*). In `v4` we got the Pattern NTM working, but it wasn't using the new memory ring. So we changed some of the nonlinearities and had the interpolation factor generated by sigmoid rather than softmax. We're trialing hard sigmoid even in the NTM.
